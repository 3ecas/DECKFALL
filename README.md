...
- **Block and Armor stop only direct hits:** attack cards, spells and a creature's own blows. Poison, Burn, Thorns, traps, summons and machines ignore them, on both sides, so a shell is no answer to a swarm or a slow poison.
...
- **One long game:** your Mana and your passives carry from fight to fight; every fight opens with a fresh shuffle of your whole deck and a new hand of five. Exhausted cards come back after the fight; enemy dispel knocks a passive out for that fight only.
...
- **Creature abilities:** each enemy and boss owns two or three abilities defined as real cards in `js/data/foecards.js` (`fc(owner, ...)`). It gains one Mana a turn and casts one of them when it has three (bosses start with one), attacking plainly in between, so a tank cannot raise its shell every turn; the pips on its card show the charge and its intent shows what comes next. It plays them between plain attacks, and when slain may drop one (30%, elites 60%, bosses always). Those cards never appear in offers: the creature is the only source. Fights are usually one creature; a second is rare and a third rarer.
...
- **Every run starts with five basic cards of one nature type:** a random element each run, two copies of its basic attack, its second attack if it has one, then its basic spells, shields and skills. Every element has a free basic attack (Claw, Tide Slap, Frost Jab, Shock Fist, Thorn Whip, Stone Fist, Radiant Blow join Strike, Torch Swing and the others), so any deck can open a run, and offers lean toward it from the first fight. Each fight opens with a fresh shuffle and five cards in hand; each turn you draw one more, or two if your hand is empty, and unplayed cards stay in your hand until the fight ends.
