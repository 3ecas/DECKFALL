...
- **Tiers:** basic → common → uncommon → medium → good → great → rare → perfect → ultimate. Basic cards do exactly one plain thing (Strike: 3 damage). Getting a card you already own evolves it one tier and multiplies its numbers (an ultimate-evolved basic is 4.2× its base), and every tier adds at least +1 to each value, so an upgrade always shows a bigger number. The smith and the forge show the card before and after, side by side, and ask before reforging. The card frame grows with the tier: a plain line on basic, rivets, a double line, corner brackets, a filigree line, gold on ultimate. Some cards are born ultimate.
