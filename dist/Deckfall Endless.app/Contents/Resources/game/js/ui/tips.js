...
const TIP=(()=>{
  ...
  function explain(f,v,d,foe){
    const t=f[0]; const n=k=>`<b>${v[k]!=null?v[k]:'?'}</b>`; const you=foe?'it':'you'; const your=foe?'its':'your'; const tgt=foe?'you':'the target'; const el=elName(d.el).toLowerCase();
    if(t==='dmg'){ const o=f[2]||{}; const hits=o.hits?(typeof o.hits==='string'?v[o.hits]:o.hits):1; let s=`Deals ${n(f[1])} ${el} damage${hits>1?` <b>${hits} times</b>`:''} to ${o.aoe&&!foe?'every enemy':tgt}`; const ex=[]; if(o.pierce) ex.push('ignores Block'); if(o.ls) ex.push(`heals ${you} for ${o.ls}% of the damage`); if(o.bv) ex.push(`double damage if the target is ${ST[o.bv].n}`); if(ex.length) s+=' · '+ex.join(' · '); s+='.'; if(!foe&&d.el!=='phys'){ const sv=strongVs(d.el), rb=resistedBy(d.el); if(sv.length) s+=` <span class="sub">2× against ${sv.join(', ')}.</span>`; if(rb.length) s+=` <span class="sub">Halved by ${rb.join(', ')}.</span>`; } return s; }
  ...
  function cardTip(id,tier){
    ...
    const lines=d.fx.map(f=>explain(f,v,d,false)).filter(Boolean); const kw=keywords(d);
    return `<h4>${d.icon} ${esc(d.name)}</h4><div class="sub">${cap(tn)} ${TYPES[d.type]} · ${elName(d.el)} · ${paysMana?`costs <b>${d.cost}</b> Mana`:'free to play'}${d.drop?` · ability of the ${esc(foeName(d.drop))}`:''}</div><div class="sub">${TYPE_DESC[d.type]}</div>${lines.length||kw.length?`<ul>${lines.map(l=>`<li>${l}</li>`).join('')}${kw.map(l=>`<li>${l}</li>`).join('')}</ul>`:''}`;
  }
  function enemyTip(e){
